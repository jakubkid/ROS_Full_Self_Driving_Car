...
class TLClassifierReal(object):
    ...
    def load_img_and_label(self, path):
        ...
        cvImg = self.preprocess_input_real(cvImg)
        return cvImg, label


    def LeNet_real(self, x):
        # Arguments used for tf.truncated_normal, randomly defines variables for the weights and biases for each layer
        mu = 0
        sigma = 0.1

        # Layer 1: Convolutional. Input = 60x144x3 . Output = 56x140x6
        ...
        # Pooling. Input = 56x140x6. Output = 28x70x6.
        filter_shape = [1, 2, 2, 1]
        strides = [1, 2, 2, 1]
        padding = 'VALID'
        layer1 = tf.nn.max_pool(layer1, filter_shape, strides, padding)

        # Layer 2: Convolutional. Input = 28x70x6.  Output = 20x62x16.
        ...
        # Pooling. Input = 20x62x16. Output = 10x31x16.
        filter_shape = [1, 2, 2, 1]
        strides = [1, 2, 2, 1]
        padding = 'VALID'
        layer2 = tf.nn.max_pool(layer2,filter_shape, strides, padding)
        # Flatten. Input = 10x31x16. Output = 4960.
        layer2 = flatten(layer2)
        # Layer 3: Fully Connected. Input = 4960. Output = 120.
        layer3Weights = tf.Variable(tf.truncated_normal([4960, 120], mu, sigma))
        ...
        logits = tf.add(tf.matmul(layer4, layer5Weights), layer5Bias, name='logitReal')
        return logits
    ...
    def train_model(self):
        ...
        xTrain = []
        yTrain = []
        trainingList = self.list_png_paths(self.filePath + '/Training/realImg/')
        ...
        xVal = []
        yVal = []
        trainingList = self.list_png_paths(self.filePath + '/Validation/realImg/')
        ...
        for path in trainingList:
            ...
            yVal.append(label)
        #load the model
        x = tf.placeholder(tf.float32, (None, 60, 144, 3), name='xReal') #input size
        ...
        with tf.Session() as sess:
            ...
            save_path = saver.save(sess, self.filePath + '/trainedModel/real.ckpt')
            print("Model saved in path: %s" % save_path)


    def get_classification(self, image):
        image = self.preprocess_input_real(image)
        images = [image]
        classificationOutput = self.sessReal.run(self.logitsReal, feed_dict={self.xReal: images})
